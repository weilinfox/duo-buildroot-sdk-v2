...
{
  echo "Usage:"
  echo "${BASH_SOURCE[0]} -h    - Show this menu"
  echo "${BASH_SOURCE[0]} build - Build [board] directly, supported boards as follows:"
...
MILKV_BOARD=milkv-duo-musl-riscv64-sd

if [ $# -lt 1 ] || ( [ "$1" != "build" ] && [ "$1" != "conf" ] ); then
#if [ $# -ge 1 ]; then
#  if [ "$1" = "lunch" ]; then
#    choose_board || exit 0
#  else
